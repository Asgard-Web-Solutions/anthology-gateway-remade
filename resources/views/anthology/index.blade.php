                ...
                <tbody class="text-gray-700">
                    @foreach ($anthologies as $anthology)
                        <tr class="hover:bg-gray-100">
                            <td class="px-6 py-4 border-b border-grey-light"><a href="{{ route('anthology.view', $anthology->id) }}" class="underline">{{ $anthology->name }}</a></td>
                            <td class="hidden px-6 py-4 border-b border-grey-light sm:table-cell">{{ $anthology->creator->email }}</td>
                            <td class="hidden px-6 py-4 border-b border-grey-light sm:table-cell"> {{ $anthology->status }}</td>
